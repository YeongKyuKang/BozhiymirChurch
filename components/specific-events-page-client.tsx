// yeongkyukang/bozhiymirchurch/BozhiymirChurch-3007c4235d54890bd3db6acc74558b701965297b/components/specific-events-page-client.tsx
...
interface SpecificEventsPageClientProps { // 인터페이스 이름 변경
  initialEvents: Event[];
  initialContent: Record<string, any>;
}

export default function SpecificEventsPageClient({ initialEvents, initialContent }: SpecificEventsPageClientProps) { // 컴포넌트 이름 변경
  ...
  const handleSaveAll = async () => {
    ...
    if (updateCount > 0) {
      try {
        const revalidateResponse = await fetch(`/api/revalidate?secret=${process.env.NEXT_PUBLIC_MY_SECRET_TOKEN}&path=/events`); // 경로를 '/events'로 유지
      ...
      <section className="py-16 px-4 pt-32">
        <div className="container mx-auto text-center">
          <h1 className="text-5xl font-bold text-gray-900 mb-6">
            ...
            />
            <span className="text-blue-600">Specific Events</span> {/* 텍스트 변경 */}
          </h1>
              ...
              <Card key={event.id} className="hover:shadow-lg transition-shadow duration-300 overflow-hidden">
                <CardContent className="p-0">
                  ...
                  <div className="p-6">
                    ...
                    <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={() => handleLearnMoreClick(event.id)}>Learn More</Button> {/* Learn More 버튼 클릭 시 상세 페이지로 이동 */}
                  </div>
                </CardContent>
              </Card>
            ))}
