// components/word-page-desktop.tsx
"use client";
...
interface WordPageDesktopProps {
  initialContent: Record<string, any>;
  initialWordPosts: WordPost[];
}

export default function WordPageDesktop({ initialContent, initialWordPosts }: WordPageDesktopProps) {
  ...
  const handleLike = async (postId: string) => {
    ...
    if (existingLike) {
      if (existingLike.reaction_type === "like") { // 'like' 타입만 처리 (좋아요 취소)
      ...
      } else { // 다른 반응 타입이 있다면 'like'로 업데이트
        const { error } = await supabase
          .from('word_reactions')
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white">
      ...
      {/* Hero Section */}
      <section className="py-8 px-4 pt-20 text-center"> 
        <div className="container mx-auto">
          ...
          <div className="text-base text-gray-600 max-w-2xl mx-auto mb-6"> 
            <EditableText
              page="word" 
              section="main"
      ...
      <section className="py-6 px-4"> 
        <div className="container mx-auto max-w-xl space-y-6 flex flex-col items-center"> 
          {/* 새로운 그리드 컨테이너: 말씀 카드와 달력을 나란히 배치 */}
          <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1fr_auto] gap-6 items-start"> 
            {/* 말씀 게시물 목록 컨테이너 (좌측 컬럼) */}
            <div className="w-full space-y-6"> 
              {wordPosts.length === 0 ? (
                <Card className="shadow-sm rounded-lg border bg-card text-card-foreground p-5 text-center py-10 w-full max-w-[200px] mx-auto"> 
                ...
                wordPosts.map(post => (
                  <Card key={post.id} ref={post.imageContainerRef} className="relative shadow-sm rounded-lg border bg-card text-card-foreground hover:shadow-lg transition-shadow duration-300 overflow-hidden w-full max-w-[200px] mx-auto"> 
                    {post.image_url ? (
                      <div className="word-card-content relative w-full aspect-[9/16] max-h-[60vh] flex items-center justify-center bg-gray-200 overflow-hidden"> 
                        <img
                          src={post.image_url}
                          alt={`말씀카드 - ${post.title}`}
                        ...
                        />
                        <div className="absolute inset-0 bg-black/70 flex flex-col justify-center items-center p-1 text-white text-center"> 
                          <CardTitle className="text-lg font-extrabold mb-0.5 break-words"> 
                            {post.title}
                          </CardTitle>
                    ...
                    {/* 좋아요, 다운로드, 읽음 버튼 */}
                    <div className="flex justify-between items-center px-1 pb-0.5 pt-0"> 
                      <div className="flex space-x-0.5"> 
                        <Button variant="ghost" size="sm" onClick={() => handleLike(post.id)} disabled={!user} className="flex items-center space-x-0.5 px-0.5 py-0"> 
                          <Heart className={`h-2.5 w-2.5 ${(post.likes ?? []).some(l => l.user_id === user?.id) ? 'text-red-500 fill-current' : 'text-gray-500'}`} /> 
                          <span className="text-[0.65rem]">{(post.likes ?? []).length}</span> 
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleDownload(post)} className="flex items-center space-x-0.5 px-0.5 py-0"> 
                          ...
                          <span className="text-[0.65rem]">다운로드</span> 
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleMarkAsRead(post.id)} disabled={!user} className="flex items-center space-x-0.5 px-0.5 py-0"> 
                          ...
                          <span className="text-[0.65rem]">읽음</span>
                        </Button>
                      </div>
                    </div>
                  </Card>
            ...
            <div className="w-full md:w-auto flex justify-center"> 
                <Card className="shadow-sm rounded-lg border bg-card text-card-foreground p-2 hover:shadow-lg transition-shadow duration-300 max-w-[200px] mx-auto"> 
                    <CardHeader className="mb-1 p-0"> 
                        <CardTitle className="flex items-center text-sm font-bold text-gray-900"> 
                            <CalendarIcon className="h-3.5 w-3.5 mr-1 text-blue-600" /> 
                            말씀 달력
                        </CardTitle>
                        <CardDescription className="text-xs">날짜를 선택하여 해당 날짜의 말씀을 확인하세요.</CardDescription> 
                    </CardHeader>
